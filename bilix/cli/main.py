...
def init_proxy(
    ...
    if not value or ctx.resilient_parsing:
        return
    from ..download.utils import init_proxy_pool

    init_proxy_pool(
        clash_api=ctx.params.get("clash_api", "http://127.0.0.1:9097"),
        proxy_server=ctx.params.get("proxy_server", "127.0.0.1"),
        proxy_port=ctx.params.get("proxy_port", 7890),
        secret=ctx.params.get("secret", "set-your-secret"),
    )
    logger.info("代理池已初始化")
...
def print_help():
    ...
    table.add_row(
        "-nh --no-hierarchy", "", "不使用层次目录，所有视频统一保存在下载目录下"
    )
    table.add_row("--image", "", "下载视频封面")
    table.add_row(
        "--subtitle",
        "",
        "下载srt字幕",
...
@click.option("--codec", "codec", type=str, default="")
@click.option(
    "--speed-limit",
    "-sl",
    "speed_limit",
...
@click.option("--stream-retry", "-sr", "stream_retry", type=int, default=5)
@click.option(
    "--from-browser",
    "-fb",
    "browser",
...
@click.option(
    "--proxy/--no-proxy", is_flag=True, callback=init_proxy, help="Enable proxy pool"
)
@click.option(
    "--clash-api", type=str, default="http://127.0.0.1:9097", help="Clash API URL"
)
@click.option(
    "--proxy-server", type=str, default="127.0.0.1", help="Proxy server address"
)
@click.option("--proxy-port", type=int, default=7890, help="Proxy server port")
@click.option(
    "--proxy-secret", type=str, default="set-your-secret", help="Clash API secret"
)
def main(**kwargs):
