...
class BaseDownloader(metaclass=BaseDownloaderMeta):
    ...
    def __init__(
        ...
        *,
        ...
        self.client = (
            client
            if client
            else httpx.AsyncClient(headers={"user-agent": "PostmanRuntime/7.29.0"})
        )
    ...
    async def get_static(
        self, url: str, path: Union[str, Path], convert_func=None
    ) -> Path:
        """
    ...
    async def _stream_context(self, times: int):
        ...
        try:
            ...
            if e.response.status_code == 403:
                ...
                await asyncio.sleep(10.0 * (times + 1))
            else:
                self.logger.warning(f"STREAM {e}")
                await asyncio.sleep(0.5 * (times + 1))
            raise
            ...
            self.logger.warning(
                f"STREAM Unexpected Exception class:{e.__class__.__name__} {e}"
            )
            raise
        finally:
            self._stream_num -= 1
    ...
    async def _check_speed(self, content_size):
        if (
            self.speed_limit
            and (cur_speed := self.progress.active_speed) > self.speed_limit
        ):
    ...
    def update_cookies_from_browser(self, browser: str):
        try:
            ...
            n_browser = getattr(browser_cookie3, browser.lower())
            self.logger.debug(
                f"trying to load cookies from {browser}: {self.cookie_domain}, may need auth"
            )
            cookies = n_browser(domain_name=self.cookie_domain)
