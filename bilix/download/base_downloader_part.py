...
class BaseDownloaderPart(BaseDownloader):
    ...
    async def _pre_req(self, urls: List[str]) -> Tuple[int, str]:
        ...
        res = await req_retry(
            self.client, urls[0], follow_redirects=True, headers={"Range": "bytes=0-1"}
        )
    ...
    async def get_media_clip(
        ...
        :return:
        """
        upper = task_id is not None and self.progress.tasks[task_id].fields.get(
            "upper", None
        )
        ...
        urls = (
            ...
            else [url for url in url_or_urls]
        )
        ...
        if task_id is not None:
            await self.progress.update(
                task_id,
                total=self.progress.tasks[task_id].total + total
        ...
        async def get_seg(part_range: Tuple[int, int]):
            async with p_sema:
                return await self._get_file_part(
                    urls, path=path, part_range=part_range, task_id=task_id
                )
        ...
        if set_s:
            await ffmpeg.time_range_clip(
                path_tmp, start=0, t=end_time - start_time + s, output_path=path
            )
        else:
            await ffmpeg.time_range_clip(
                path_tmp, start=s, t=end_time - start_time, output_path=path
            )
    ...
    async def get_file(
        ...
        if path.is_dir():
            file_name = (
                req_filename if req_filename else PurePath(urlparse(urls[0]).path).name
            )
            path /= file_name
            exist, path = path_check(path)
            if exist:
        ...
        for i in range(self.part_concurrency):
            start = i * part_length
                ...
                (i + 1) * part_length - 1
                if i < self.part_concurrency - 1
                else total - 1
            )
            cors.append(
                self._get_file_part(
                    urls, path=path, part_range=(start, end), task_id=task_id
                )
            )
    ...
    async def _get_file_part(
        self, urls: List[str], path: Path, part_range: Tuple[int, int], task_id
    ) -> Path:
        ...
        for times in range(1 + self.stream_retry):
            try:
                ...
                ) as r, self._stream_context(times), aiofiles.open(
                    part_path, "ab"
                ) as f:
                    r.raise_for_status()
