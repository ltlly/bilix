...
async def get_list_info(
    ...
    if url_or_sid.startswith("http"):
        ...
        sid = url_or_sid
    res = await req_retry(
        client, f"https://api.bilibili.com/x/series/series?series_id={sid}"
    )  # meta api
...
async def get_collect_info(client: httpx.AsyncClient, url_or_sid: str):
    ...
    res = await req_retry(
        client, "https://api.bilibili.com/x/space/fav/season/list", params=params
    )
...
@raise_api_error
async def get_favour_page_info(
    client: httpx.AsyncClient, url_or_fid: str, pn=1, ps=20, keyword=""
):
    """
    获取收藏夹信息（分页）
    ...
    res = await req_retry(
        client, "https://api.bilibili.com/x/v3/fav/resource/list", params=params
    )
...
async def get_cate_page_info(
    ...
    res = await req_retry(
        client, "https://s.search.bilibili.com/cate/search", params=params
    )
...
async def _add_sign(client: httpx.AsyncClient, params: dict):
    ...
    res = await req_retry(client, "https://api.bilibili.com/x/web-interface/nav")
    info = json.loads(res.text)
...
@raise_api_error
async def get_up_video_info(
    client: httpx.AsyncClient, url_or_mid: str, pn=1, ps=30, order="pubdate", keyword=""
):
    """
    获取up主信息
...
async def get_up_info(client: httpx.AsyncClient, url_or_mid: str):
    ...
    await _add_sign(client, params)
    res = await req_retry(
        client, "https://api.bilibili.com/x/space/wbi/acc/info", params=params
    )
    data = json.loads(res.text)["data"]
    return data
...
class Dash(BaseModel):
    ...
    def choose_quality(
        self, quality: Union[str, int], codec: str = ""
    ) -> Tuple[Media, Optional[Media]]:
...
async def _parse_bv_html(client: httpx.AsyncClient, url, html: str) -> VideoInfo:
    init_info = re.search(
        r"<script>window.__INITIAL_STATE__=({.*?});\(", html
    ).groups()[0]  # this line may raise
    init_info = json.loads(init_info)
    if len(init_info.get("error", {})) > 0:
        raise APIResourceError(
            "视频已失效", url
        )  # 啊叻？视频不见了？在分区下载的时候可能产生
    # extract meta
    pages = []
    ...
    try:
        for i in play_info["durl"]:
            ...
            other.append(
                Media(base_url=i["url"], backup_url=i["backup_url"], suffix=suffix)
            )
    except KeyError:
        pass
    # extract img url
    ...
    if not img_url.startswith(
        "http"
    ):  # https://github.com/HFrost0/bilix/issues/52 just for some video
...
def _parse_ep_html(url, html: str) -> VideoInfo:
    ...
    status = Status(
        coin=stat["coins"],
        view=stat["views"],
        danmaku=stat["danmakus"],
        share=stat["share"],
        like=stat["likes"],
        reply=stat["reply"],
        favorite=stat["favorite"],
        follow=stat["favorites"],
    )
...
async def _attach_ep_dash(client: httpx.AsyncClient, video_info: VideoInfo):
    ...
    res = await req_retry(
        client, "https://api.bilibili.com/pgc/player/web/v2/playurl", params=params
    )
...
async def _attach_dash_and_durl_from_api(
    client: httpx.AsyncClient, video_info: VideoInfo
):
    params = {
        "cid": video_info.cid,
...
async def _get_video_basic_info_from_api(client: httpx.AsyncClient, url) -> VideoInfo:
    ...
    assert p is not None, (
        f"没有找到分P: p{selected_page_num}，请检查输入"
    )  # cid 也会是 None
    img_url = raw_json["data"]["pic"]
    basic_video_info = VideoInfo(
        title=title,
        h1_title=h1_title,
        aid=aid,
        cid=cid,
...
async def get_subtitle_info(client: httpx.AsyncClient, bvid, cid):
    ...
    return [
        [f"http:{i['subtitle_url']}", i["lan_doc"]]
...
async def get_dm_urls(client: httpx.AsyncClient, aid, cid) -> List[str]:
    ...
    res = await req_retry(
        client, f"https://api.bilibili.com/x/v2/dm/web/view", params=params
    )
    view = parse_view(res.content)
    total = int(view["dmSge"]["total"])
    return [
        f"https://api.bilibili.com/x/v2/dm/web/seg.so?oid={cid}&type=1&segment_index={i + 1}"
