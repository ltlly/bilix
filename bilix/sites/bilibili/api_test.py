...
async def test_get_list_info():
    list_name, up_name, bvids = await api.get_list_info(
        client, "https://space.bilibili.com/369750017/channel/seriesdetail?sid=2458228"
    )
...
async def test_get_cate_page_info():
    ...
    bvids = await api.get_cate_page_info(
        client, cate_id=meta["宅舞"]["tid"], time_from=time_from, time_to=time_to
    )
    assert len(bvids) > 0 and bvids[0].startswith("BV")
...
async def test_get_up_video_info():
    up_name, total_size, bvids = await api.get_up_video_info(
        client, "316568752", keyword="什么"
    )
    assert len(bvids) > 0 and bvids[0].startswith("BV")
...
async def test_get_video_info():
    ...
    for method in methods:
        ...
        data = await method(
            client,
            "https://www.bilibili.com/video/BV1sS4y1b7qb?spm_id_from=333.999.0.0",
        )
        ...
        data = await method(
            client,
            "https://www.bilibili.com/bangumi/play/ss24053?spm_id_from=333.337.0.0",
        )
        assert len(data.pages) > 1
        ...
        data = await method(
            client,
            "https://www.bilibili.com/bangumi/play/ss5043?spm_id_from=333.337.0.0",
        )
        assert len(data.pages) > 1
        assert data.status.follow
        # 电影
        ...
        data = await method(
            client,
            "https://www.bilibili.com/bangumi/play/ss40509?from_spmid=666.9.hotlist.3",
        )
        assert len(data.pages) > 1
        assert data.status.follow
...
async def test_get_subtitle_info():
    data = await api.get_video_info(
        client, "https://www.bilibili.com/video/BV1hS4y1m7Ma"
    )
