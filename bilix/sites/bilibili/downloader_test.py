...
async def test_get_series():
    d = DownloaderBilibili()
    await d.get_series(
        "https://www.bilibili.com/video/BV1jK4y1N7ST?p=3", p_range=(5, 5), quality=999
    )
    # only audio
    await d.get_series(
        "https://www.bilibili.com/video/BV1jK4y1N7ST?p=3",
        p_range=(5, 5),
        only_audio=True,
    )
    # image
...
async def test_get_dm():
    d = DownloaderBilibili()
    await d.get_dm(
        "https://www.bilibili.com/video/BV11Z4y1z7s8?spm_id_from=333.337.search-card.all.click"
    )
    await d.aclose()
...
async def test_choose_quality():
    ...
    data = await api.get_video_info(
        client, "https://www.bilibili.com/video/BV13L4y1K7th"
    )
    try:
    ...
    # hi-res
    data = await api.get_video_info(
        client, "https://www.bilibili.com/video/BV16K411S7sk"
    )
    try:
