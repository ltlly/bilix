...
def parse_ids_from_url(url_or_string: str):
    ...
    if re.match(r"https?://www.bilibili.com/video/BV\w+", url_or_string) or re.match(
        r"BV\w+", url_or_string
    ):
        bvid = re.search(r"(BV\w+)", url_or_string).groups()[0]
        assert bvid.isalnum()
    elif re.match(r"https?://www.bilibili.com/video/av\d+", url_or_string) or re.match(
        r"av\d+", url_or_string
    ):
