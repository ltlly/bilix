...
async def get_media_info(
    ...
    """
    res = await req_retry(
        client, f"https://vdn.apps.cntv.cn/api/getHttpVideoInfo.do?pid={pid}"
    )
    ...
    m3u8_urls = list(
        sorted(
            (i.absolute_uri for i in m3u8_info.playlists),
            reverse=True,
...
async def get_series_info(
    client: httpx.AsyncClient, vide: str, vida: str
) -> Tuple[str, Sequence[str]]:
    """
    ...
    res_meta, res_list = await asyncio.gather(
        req_retry(
            client,
            f"https://api.cntv.cn/NewVideoset/getVideoAlbumInfoByVideoId?id={vide}&serviceId=tvcctv",
        ),
        req_retry(
            client,
            f"https://api.cntv.cn/NewVideo/getVideoListByAlbumIdNew",
            params=params,
        ),
    )
