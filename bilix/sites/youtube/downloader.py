    ...
    def __init__(
        ...
        *,
        ...
        self.video_sema = (
            ...
            if type(video_concurrency) is int
    ...
    async def get_video(self, url: str, path=Path(".")):
        ...
        async with self.video_sema:
            ...
            task_id = await self.progress.add_task(
                description=video_info.title, upper=True
            )
            path_lst = await asyncio.gather(
                self.get_file(
                    url_or_urls=video_info.video_url,
                    path=path / (video_info.title + "-v"),
                    task_id=task_id,
                ),
                self.get_file(
                    url_or_urls=video_info.audio_url,
                    path=path / (video_info.title + "-a"),
                    task_id=task_id,
                ),
            )
